...
for source_file in ${source_files}; do
  ...
  sed -i 's/ ["]*image["]*: [a-z0-9\-\."]*\// image: platform-harbor.mvp.bsl.local\/kubeflow\//g'  $CONVERTED_FOLDER/$filename
  ## image: docker.io/a/b:v1 ==> image: platform-harbor.mvp.bsl.local/a/b:v1
  ...
  while IFS= read -r image_repo_domain; do
    ...
    command="sed -i 's/image: ${image_repo_domain}/image: platform-harbor.mvp.bsl.local\/kubeflow/g' $CONVERTED_FOLDER/$filename"
    eval $command
